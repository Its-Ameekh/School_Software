...
export default function ParentDashboard() {
  ...
  const [activeTab, setActiveTab] = useState('Dashboard'); // 'Dashboard', 'Academics', 'Gallery'
  
  // --- UTILITY NAVIGATION DROPDOWNS ---
  ...
  return (
    <div className="bg-[#f9f9ff] text-[#111c2c] min-h-screen font-sans antialiased relative overflow-x-hidden pb-12">
      ...
      {/* --- TOP HEADER NAVIGATION W/ INTERACTIVE MENUS --- */}
        ...
        <div className="flex justify-between items-center w-full px-6 md:px-10 h-20 max-w-[1280px] mx-auto relative">
          ...
          <div className="hidden md:flex items-center gap-8">
            <nav className="flex gap-4 font-medium text-sm">
              <button onClick={() => setActiveTab('Dashboard')} className={`px-4 py-1.5 rounded-full transition font-semibold ${activeTab === 'Dashboard' ? 'text-[#005fb0] bg-blue-50' : 'text-gray-500 hover:bg-gray-50'}`}>Dashboard</button>
              <button onClick={() => setActiveTab('Academics')} className={`px-4 py-1.5 rounded-full transition font-semibold ${activeTab === 'Academics' ? 'text-[#005fb0] bg-blue-50' : 'text-gray-500 hover:bg-gray-50'}`}>Academics</button>
              <button onClick={() => setActiveTab('Gallery')} className={`px-4 py-1.5 rounded-full transition font-semibold ${activeTab === 'Gallery' ? 'text-[#005fb0] bg-blue-50' : 'text-gray-500 hover:bg-gray-50'}`}>Gallery</button>
            </nav>
            
            <div className="flex items-center gap-4 border-l border-gray-200 pl-6 text-gray-400 text-lg relative">
              {/* Notification Trigger Menu */}
              <button onClick={() => { setIsNotifOpen(!isNotifOpen); setIsMessagesOpen(false); }} className={`hover:text-gray-600 transition cursor-pointer relative p-1 rounded-full ${isNotifOpen ? 'text-gray-700 bg-gray-100' : ''}`}>
                🔔<span className="absolute top-0 right-0 w-2 h-2 bg-red-500 rounded-full"></span>
              </button>
              
              {/* Message Icon Trigger Menu */}
              <button onClick={() => { setIsMessagesOpen(!isMessagesOpen); setIsNotifOpen(false); }} className={`hover:text-gray-600 transition cursor-pointer relative p-1 rounded-full ${isMessagesOpen ? 'text-gray-700 bg-gray-100' : ''}`}>
                ✉<span className="absolute top-0 right-0 w-2 h-2 bg-blue-500 rounded-full"></span>
              </button>
              
              <button onClick={() => alert('Opening Parent Portal Configuration Settings Panel...')} className="hover:text-gray-600 transition cursor-pointer p-1">⚙</button>
              
              <div className="w-10 h-10 rounded-full overflow-hidden border-2 border-blue-400 ml-2 shadow-sm">
              ...
              {/* DYNAMIC NAV MENU OVERLAYS */}
              {isNotifOpen && (
                <div className="absolute top-12 right-24 w-64 bg-white border border-gray-100 shadow-2xl rounded-2xl p-4 text-xs z-50 text-gray-700 border-t-4 border-t-yellow-400 animate-in fade-in slide-in-from-top-2 duration-150">
                  <p className="font-bold mb-2 border-b pb-1">🔔 Live Updates</p>
                  <p className="text-gray-500 italic py-1">● Bus 14 has entered your tracking perimeter zone.</p>
                  <p className="text-gray-500 italic py-1">● Art assignment grade metric updated by teacher.</p>
                </div>
              )}

              {isMessagesOpen && (
                <div className="absolute top-12 right-14 w-66 bg-white border border-gray-100 shadow-2xl rounded-2xl p-4 text-xs z-50 text-gray-700 border-t-4 border-t-blue-500 animate-in fade-in slide-in-from-top-2 duration-150">
                  <p className="font-bold mb-2 border-b pb-1">✉ Unread Inbox Channels</p>
                  <p className="font-semibold text-[#005fb0]">Ms. Sarah J. (Class Teacher)</p>
                  <p className="text-gray-400 truncate mb-2">"Aiden left his sketchbook on the art table..."</p>
                  <p className="font-semibold text-gray-700">Office Administration</p>
                  <p className="text-gray-400 truncate">"Q2 receipt verification confirmation notice..."</p>
                </div>
              )}
            </div>
          </div>
      ...
      {/* --- MAIN CORE VIEWSPACE CONTENT CHANGER --- */}
      <main className="max-w-[1280px] mx-auto px-4 md:px-10 py-8 space-y-8">
        
        {/* TAB CONTROLLER ROUTER CONDITIONAL SWITCH */}
        {activeTab !== 'Dashboard' ? (
          <section className="bg-white rounded-3xl p-12 text-center border border-gray-100 shadow-sm animate-in fade-in duration-200">
            <span className="text-5xl block mb-4">📂</span>
            <h3 className="text-xl font-bold text-gray-800">Starlight {activeTab} Records</h3>
            <p className="text-gray-400 text-xs mt-1 max-w-sm mx-auto">This sub-module tab is securely prepared! When your teammates push the teacher portal sync updates, real-time reports will populate here.</p>
            <button onClick={() => setActiveTab('Dashboard')} className="mt-4 px-4 py-2 bg-[#005fb0] text-white rounded-full text-xs font-semibold">Return to Dashboard</button>
          </section>
        ) : (
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
              ...
              <div className="lg:col-span-4 rounded-2xl p-6 bg-white border border-gray-100 shadow-sm flex flex-col items-center relative overflow-hidden">
                ...
                {/* 📞 INTEGRATED CONTACT SYSTEM PROTOCOLS */}
                <div className="mt-6 w-full border-t border-gray-100 pt-6">
                  <p className="text-sm font-bold mb-3">Contact Support</p>
                  <div className="space-y-2">
                    {renderContactRow("School Office", "tel:+919876543210")}
                    {renderContactRow("Ms. Sarah J.", "tel:+919876543211")}
                  </div>
                </div>

                <button onClick={() => setActiveTab('Gallery')} className="w-full mt-6 py-3 rounded-full border-2 border-blue-200 text-[#005fb0] font-semibold text-sm hover:bg-blue-50/50 transition flex items-center justify-center gap-2">
                  🖼 Gallery Highlights
                </button>
              </div>

              {/* --- RIGHT COLUMN: BENTO VIEW TILES --- */}
              <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="rounded-2xl p-6 bg-white border border-gray-100 shadow-sm flex flex-col justify-between">
                  ...
                  <div className="border-2 border-dashed border-gray-200 rounded-xl p-4 text-center cursor-pointer hover:border-blue-400 transition mt-4 bg-gray-50/50" onClick={() => alert('Downloading Academic Summary Document Asset PDF...')}>
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">⬇ Download Report Card (PDF)</p>
                  </div>
                </div>

                {/* Field Trip Event Announcement Block */}
                <div className="rounded-2xl p-6 bg-white border border-gray-100 shadow-sm flex flex-col justify-between">
                  <div className="flex justify-between items-start">
                    <div>
            ...
            <section className="rounded-2xl p-6 bg-white border border-gray-100 shadow-sm space-y-4">
              <div className="flex justify-between items-center">
                ...
                <button onClick={() => setActiveTab('Gallery')} className="text-sm text-[#005fb0] font-semibold hover:underline inline-flex items-center gap-1">Full Gallery →</button>
              </div>
...
// --- RENDERING TEMPLATE PIPELINES W/ CALL LINKS ---
function renderContactRow(title, telLink) {
  return (
    <div className="flex justify-between items-center bg-gray-50 p-3 rounded-xl hover:bg-gray-100/70 transition">
      <div>
        ...
        <p className="text-xs font-semibold text-gray-700 mt-0.5">Verified Profile</p>
      </div>
      <div className="flex gap-1.5">
        <a href={telLink} className="w-7 h-7 bg-white rounded-full border border-gray-200 text-xs flex items-center justify-center shadow-sm hover:bg-blue-50 active:scale-90 transition">📞</a>
        <button onClick={() => alert(`Starting text correspondence thread with ${title}...`)} className="w-7 h-7 bg-white rounded-full border border-gray-200 text-xs flex items-center justify-center shadow-sm hover:bg-blue-50 active:scale-90 transition">💬</button>
      </div>
    </div>
  );
}
      ...
      return (
        <div className="md:col-span-2 rounded-2xl p-6 bg-white border border-gray-100 shadow-sm">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gray-50 p-4 rounded-xl">
            ...
            {/* Real smartphone tel loop protocol tag */}
            <a href="tel:+919876543212" className="px-5 py-2.5 bg-[#005fb0] text-white text-xs rounded-full font-semibold hover:bg-blue-700 transition shadow-sm text-center active:scale-95">
              📞 Call Driver
            </a>
          </div>
        </div>
